...
const TABS: StatusFilter[] = ["all", "upcoming", "pending"];
const TAB_LABEL: Record<string, string> = { all: "All", upcoming: "Upcoming", pending: "Pending" };
...
function HomeContent() {
  ...
  const visible = useMemo(
    () => (filter === "all" ? items : items.filter((item) => computeDisplayStatus(item) === filter)),
    [items, filter],
  );
    ...
    <main className="min-h-screen px-4 py-5 sm:px-8 sm:py-8 lg:px-12">
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-6 border-b border-[var(--line)] pb-7 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--muted)]">Welcome{user ? `, ${user.name}` : ""}</p>
            </div>
          </div>
          <div className="flex gap-3">
            <Link href="/appointments" className="rounded-lg border border-[var(--line)] px-4 py-2.5 text-sm font-semibold hover:border-[var(--brand)] hover:text-[var(--brand)]">
              My appointments
            </Link>
            <Link href="/doctors" className="rounded-lg bg-[var(--brand)] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[var(--brand-deep)]">
              New appointment
            </Link>
          </div>
        </header>
        ...
        <section className="py-8" aria-labelledby="dashboard-title">
          <p className="text-sm font-medium text-[var(--brand)]">Clinic overview</p>
          <div className="mt-2 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
            ...
            <p className="text-sm text-[var(--muted)]">
              <span className="font-semibold text-[var(--ink)]">{confirmedCount} confirmed</span> of {items.length} visits
            </p>
          </div>
        </section>
          ...
          <section className="overflow-hidden rounded-xl border border-[var(--line)] bg-white" aria-labelledby="schedule-title">
            ...
            {status === "loading" && (
              <div className="space-y-4 p-5" aria-busy="true" aria-label="Loading appointments">
                {[1, 2, 3].map((item) => (
                  <div className="h-20 animate-pulse rounded-lg bg-stone-100" key={item} />
                ))}
              </div>
            )}
            {status === "error" && (
              <div className="p-8 text-center" role="alert">
                ...
                <button className="mt-3 text-sm font-semibold text-[var(--brand)] underline" onClick={() => window.location.reload()} type="button">
                  Try again
                </button>
              </div>
            )}
              ...
              <ul className="divide-y divide-[var(--line)]" role="list">
                {visible.map((item) => (
                  <li key={item.id}>
                    ...
                    >
                      <time className="pt-1 text-sm font-medium text-[var(--muted)]">{time(item.startsAt)}</time>
            ...
            {status === "ready" && visible.length === 0 && (
              <div className="p-10 text-center">
                <p className="font-medium">No appointments match this filter.</p>
            ...
            {selected ? (
              <div className="mt-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="font-semibold">{selected.patient.name}</h2>
                    <p className="text-sm text-[var(--muted)]">{selected.patient.age > 0 ? `${selected.patient.age} years old` : "—"}</p>
                  </div>
                </div>
                <dl className="mt-6 space-y-4 text-sm">
                  ...
                  <div>
                    ...
                    <dd className="mt-1 font-medium">
                      {time(selected.startsAt)} · {selected.room}
                    </dd>
                  </div>
                  <div>
                    <dt className="text-[var(--muted)]">Care team</dt>
                    <dd className="mt-1 font-medium">
                      {selected.clinician}
                      <br />
                      {selected.specialty}
